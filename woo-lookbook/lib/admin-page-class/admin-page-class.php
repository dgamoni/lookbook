        ...
        if(is_object($args['menu'])) {
          ...
          switch($args['menu']) {
            ...
            case 'pages':
              ...
              break;
            //case 'woocommerce':
            //  $this->Top_Slug = 'edit.php?post_type=product';
            //  break;
            case 'comments':
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '</table>
        <span class="at-re-toggle lb_edit">edit</span> 
        <span class="lb_remove" id="remove-'.$field['id'].'">remove</span></div>';
        $c = $c + 1;
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_posts($field, $meta) {
    ...
    // select
      ...
      echo "<option value='none'" . selected( $field['id'], 'none' ) . ">none</option>";
  ...
  public function addTypoColor($id,$args,$repeater=false){
    $new_field = array(
      'type' => 'color', 
      'id'=> $id,
      'std' => array(
    ...
    if ($fonts === false){
      ...
      if ($this->google_fonts){
        $api_keys = array(
          ...
          //'AIzaSyBUmaplYOMfTjC0Pyh2e5dzHI7FA_yqMTQ'
        );
